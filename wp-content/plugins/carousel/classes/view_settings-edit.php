...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		var saved_resp_setting = "<?php echo $group['responsive'] ?>";
		if ( 'true' == saved_resp_setting ) { 
			jQuery('.hide_dir_rsp').show();
		}
		if ( 'false' == saved_resp_setting ) { 
			jQuery('.hide_dir_fx').show();
		}
		jQuery( '.settings_picker' ).click(function() {
			var is_responsive = jQuery( 'input[name="#responsive"]:checked' ).val()
			if ( 'true' == is_responsive ) { 
				jQuery('.hide_dir_rsp').show();
				jQuery('.hide_dir_fx').hide();
			}
			if ( 'false' == is_responsive ) { 
				jQuery('.hide_dir_fx').show();
				jQuery('.hide_dir_rsp').hide();
			}	
...
<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->_selfLink; ?>-settings&edit=<?php echo htmlentities( $_GET['edit'] ); ?>">
	<div class="tablenav">
		<div class="alignleft actions">
            ...
            <?php
            $link_args = array( 'title' => __( '+ Add Image', 'it-l10n-carousel' ) );
...
<form method="post" action="<?php echo $this->_selfLink; ?>-settings&edit=<?php echo htmlentities( $_GET['edit'] ); ?>">
	...
	<table class="form-table">
		...
		<tr>
			<td><label for="responsive">Enable responsive carousel<?php $this->tip( 'When enabled, the Carousel will be responsive.' ); ?></label></td>
			<td>
				<label for="responsive-1">
					<input class="radio_toggle settings_picker" type="radio" name="#responsive" id="responsive-1" value="true" onclick="jQuery('#responsivewidth, #swipeRadio').show(); jQuery('#widthStandard, .hideItems').hide();"<?php if ( $group['responsive'] == 'true' ) { echo ' checked '; } ?>/>
					Yes
				</label>
				&nbsp;
				<label for="responsive-0">
					<input class="radio_toggle settings_picker" type="radio" name="#responsive" id="responsive-0" value="false" onclick="jQuery('#responsivewidth, #swipeRadio').hide(); jQuery('#widthStandard, .hideItems').show();"<?php if ( $group['responsive'] == 'false' ) { echo ' checked '; } ?>/>
					No
				</label>
			</td>
		</tr>
		<tr class='swipeDisplay' id='swipeRadio' <?php if ( $group['responsive'] == 'false' ) { echo ' style="display: none;" '; } ?>>
			<td><label for="swipe">Enable swipe<?php $this->tip( 'When enabled, swipe navigation will be available for mobile devices.' ); ?></label></td>
			<td>
				<label for="swipe-1">
					<input class="radio_toggle" type="radio" name="#swipe" id="swipe-1" value="true" <?php if ( $group['swipe'] == 'true' ) { echo ' checked '; } ?>/>
					Yes
				</label>
				&nbsp;
				<label for="swipe-0">
					<input class="radio_toggle" type="radio" name="#swipe" id="swipe-0" value="false" <?php if ( $group['swipe'] == 'false' ) { echo ' checked '; } ?>/>
					No
				</label>
			</td>
		</tr>
	<tr class='responsiveDisplay' id='responsivewidth' <?php if ( $group['responsive'] == 'false' ) { echo ' style="display: none;" '; } ?>>
	<td><label for="responsive">Entity width<?php $this->tip('Width of the entire Carousel itself (responsive).'); ?><label></td>
	<td><label for="rentity_width"><input type="text" name="#rentity_width" id="rentity_width" size="5" maxlength="5" style="text-align: right;" value="<?php echo $group['rentity_width']; ?>" />px wide</label></td>
	</tr>
		<tr class='widthDisplay' id='widthStandard' <?php if ( $group['responsive'] == 'true' ) { echo ' style="display: none;" '; } ?>>
		...
		<tr class="hide_dir_fx" style=" display: none; " >
		...
		<tr class="hide_dir_rsp" style=" display: none; " >
			<td><label for="resp_direction">Carousel orientation & direction <?php $this->tip( 'Controls the animation direction and orientation of the Carousel.' ); ?></label></td>
			<td>
				<select name="#resp_direction" id="resp_direction">
					<option value="left" <?php if ( 'left' == $group['resp_direction'] ) { echo 'selected'; } ?> />Horizontal - Slide left</option>
					<option value="right" <?php if ( 'right' == $group['resp_direction'] ) { echo 'selected'; } ?> />Horizonal - Slide right</option>
				</select>
			</td>
		</tr>
		<tr>
		...
		<tr class="hideItems" <?php if ( $group['responsive'] == 'true' ) { echo ' style="display: none;" '; } ?>>
		...
		<tr>
			<td><label for="scroll_items">Images to scroll at once<?php $this->tip( 'Determines how many images will scroll in one transition.' ); 	?></label></td>
			<td><input type="text" name="#scroll_items" id="scroll_items" size="5" maxlength="5" value="<?php echo $group['scroll_items']; ?>" style="text-align: right;" />images</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<label for="show_navigation-1">
					<input class="radio_toggle" type="radio" name="#show_navigation" id="show_navigation-1" value="true" onclick="jQuery('.arrowStyles').show();" <?php if ( $group['show_navigation'] == 'true' ) { echo ' checked '; } ?>/>
					Yes
				</label>
				&nbsp;
				<label for="show_navigation-0">
					<input class="radio_toggle" type="radio" name="#show_navigation" id="show_navigation-0" value="false" onclick="jQuery('.arrowStyles').hide();" <?php if ( $group['show_navigation'] == 'false' ) { echo ' checked '; } ?>/>
					No
				</label>
			</td>
		</tr>
		<tr class="arrowStyles" <?php if ( $group['show_navigation'] == 'false' ) { echo ' style="display: none;" '; } ?>>
			<td><label for="arrows">Arrow navigation styles<?php $this->tip( 'Controls the arrow navigation style.' ); ?></label></td>
			<td>
				<select name="#arrows" id="arrows">
					<option value="classic_outset" <?php if ( 'classic_outset' == $group['arrows'] ) { echo 'selected'; }?> />Classic-Outset</option>
					<option value="classic_inset" <?php if ( 'classic_inset' == $group['arrows'] ) { echo 'selected'; }?> />Classic-Inset</option>
					<option value="minimal" <?php if ( 'minimal' == $group['arrows'] ) { echo 'selected'; }?> />Minimal</option>
					<option value="dark_lined" <?php if ( 'dark_lined' == $group['arrows'] ) { echo 'selected'; }?> />Dark Lined</option>
					<option value="light_lined" <?php if ( 'light_lined' == $group['arrows'] ) { echo 'selected'; }?> />Light Lined</option>
					<option value="dark_double" <?php if ( 'dark_double' == $group['arrows'] ) { echo 'selected'; }?> />Dark Double</option>
					<option value="light_double" <?php if ( 'light_double' == $group['arrows'] ) { echo 'selected'; }?> />Light Double</option>
				</select>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td><label for="open_link">Open image link in new window<?php $this->tip( 'When enabled, clicking on a image link will open the url in a new window.' ); ?></label></td>
			<td>
				<label for="open_link-1">
					<input class="radio_toggle" type="radio" name="#open_link" id="open_link-1" value="true" <?php if ( $group['open_link'] == 'true' ) { echo ' checked '; } ?>/>
					Yes
				</label>
				&nbsp;
				<label for="open_link-0">
					<input class="radio_toggle" type="radio" name="#open_link" id="open_link-0" value="false" <?php if ( $group['open_link'] == 'false' ) { echo ' checked '; } ?>/>
					No
				</label>
			</td>
		</tr>
